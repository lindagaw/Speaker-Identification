...
ubmfiles = dir(strcat(rootDirName, '\ubm-feature-func-f-25'));
speakerfiles = dir(strcat(rootDirName,'\feature-func-f-25'));
...
parfor i = 1 : times
    file = strcat(rootDirName,'\ubm-feature-func-f-25\',ubmfiles(i+2).name);
...
save(strcat(rootDirName,'/models_ubm_f_25_', num2str(nMixtures),'.mat'), 'ubm');
...
parfor i = 1 : times
    file = strcat(rootDirName,'\feature-func-f-25\',speakerfiles(i+2).name);
...
save(strcat(rootDirName,'/models_f_25_', num2str(nMixtures),'.mat'), 'models');



% Set the parameters of the experiment
ubmfiles = dir(strcat(rootDirName, '\ubm-feature-func-f-50'));
speakerfiles = dir(strcat(rootDirName,'\feature-func-f-50'));
...
parfor i = 1 : times
    file = strcat(rootDirName,'\ubm-feature-func-f-50\',ubmfiles(i+2).name);
    [numbers, ~] = swallow_csv(file);
    trainUBMData{i} = numbers';
end
% Create the universal background model from the first wav file's speaker data
...
ubm = gmm_em(trainUBMData, nmix, final_niter, ds_factor, nWorkers);
save(strcat(rootDirName,'/models_ubm_f_50_', num2str(nMixtures),'.mat'), 'ubm');
...
parfor i = 1 : times
    file = strcat(rootDirName,'\feature-func-f-50\',speakerfiles(i+2).name);
...
save(strcat(rootDirName,'/models_f_50_', num2str(nMixtures),'.mat'), 'models');
